...
const pokemonPairs: string[] = [...pokemonImages, ...pokemonImages];
...
const shuffledPokemonPairs: string[] = shuffleArray(pokemonPairs);
...
function flipCard(event: Event) {
    ...
    const clickedImage: HTMLImageElement = event.target as HTMLImageElement;
    const dataIndex: string | null = clickedImage.dataset.index;
    if (dataIndex === null) {
        return; 
    }
